...
class Point(object):
    """ Represents a 20 point in x y space """
    ...
    def closer_to(self, first, second):
        if self.get_distance_from(first) > self.get_distance_from(second):
            return second
        else:
            return first

    def halfway_to(self, point):
        return Point(self.x+(point.x - self.x)/2, self.y+(point.y - self.y)/2)
